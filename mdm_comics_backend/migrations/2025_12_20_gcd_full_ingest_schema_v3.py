"""
Migration: GCD Full Ingest Schema Expansion v3 (REBOOT)
IMP-20251220-GCD-FULL-INGEST

Robust non-transactional schema migration using explicit ALTER TABLE commands
to handle existing tables and missing columns.
"""
import asyncio
import logging
import os
...
async def run_migration():
    ...
    steps = [
        ("comic_indicia_publishers table", """
        ...
        ("comic_issues expansion", """
            ...
            BEGIN
                ...
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='comic_issues' AND column_name='gcd_brand_id') THEN
                    ALTER TABLE comic_issues ADD COLUMN gcd_brand_id INTEGER;
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='comic_issues' AND column_name='gcd_indicia_publisher_id') THEN
                    ALTER TABLE comic_issues ADD COLUMN gcd_indicia_publisher_id INTEGER;
                END IF;
            END $$;
        """),
        ("stories expansion", """
            ...
            BEGIN
                ...
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='stories' AND column_name='gcd_issue_id') THEN
                    ALTER TABLE stories ADD COLUMN gcd_issue_id INTEGER;
                END IF;
            END $$;
        """),
        ("comic_characters expansion", """
            DO $$
            BEGIN
        ...
        ("comic_reprints table", """
            CREATE TABLE IF NOT EXISTS comic_reprints (
                ...
                gcd_origin_story_id INTEGER,
                gcd_target_story_id INTEGER,
                notes TEXT,
                created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
                updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
            );
        """),
        ("Indicia GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_indicia_publishers_gcd_id ON comic_indicia_publishers(gcd_id)"),
        ("Brand GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_brands_gcd_id ON comic_brands(gcd_id)"),
        ("Publisher GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_publishers_gcd_id ON comic_publishers(gcd_id)"),
        ("Series GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_series_gcd_id ON comic_series(gcd_id)"),
        ("Issue Variant GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_issues_gcd_variant_of_id ON comic_issues(gcd_variant_of_id)"),
        ("Issue KeyDate index", "CREATE INDEX IF NOT EXISTS ix_comic_issues_key_date ON comic_issues(key_date)"),
        ("Story GCD index", "CREATE INDEX IF NOT EXISTS ix_stories_gcd_story_id ON stories(gcd_story_id)"),
        ("Story Issue index", "CREATE INDEX IF NOT EXISTS ix_stories_comic_issue_id ON stories(comic_issue_id)"),
        ("Character GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_characters_gcd_id ON comic_characters(gcd_id)"),
        ("Creator GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_creators_gcd_id ON comic_creators(gcd_id)"),
        ("Reprint GCD index", "CREATE INDEX IF NOT EXISTS ix_comic_reprints_gcd_id ON comic_reprints(gcd_id)"),
        ("Reprint Origin index", "CREATE INDEX IF NOT EXISTS ix_comic_reprints_origin_story_id ON comic_reprints(origin_story_id)"),
        ("Reprint Target index", "CREATE INDEX IF NOT EXISTS ix_comic_reprints_target_story_id ON comic_reprints(target_story_id)")
    ]

"""
Migration: GCD Full Ingest Schema Expansion v2 (IMP-20251220-GCD-FULL-INGEST)

Robust migration that handles existing 'stories' tables and adds missing GCD fields.
"""
import asyncio
import logging
import os
...
async def run_migration():
    async with AsyncSessionLocal() as db:
        try:
            logger.info("Starting GCD Full Ingest Schema Expansion v2...")
            ...
            # 6. Update existing stories table
            logger.info("Updating stories table")
            await db.execute(text("""
                DO $$
                BEGIN
                ...
                END $$;
            """))
            await db.execute(text("CREATE INDEX IF NOT EXISTS ix_stories_gcd_story_id ON stories(gcd_story_id)"))
            await db.execute(text("CREATE INDEX IF NOT EXISTS ix_stories_comic_issue_id ON stories(comic_issue_id)"))
            ...
            # 9. Update story_characters M2M
            logger.info("Updating story_characters table")
            await db.execute(text("""
                DO $$
                BEGIN
            ...
            # 10. Update story_creators M2M
            logger.info("Updating story_creators table")
            await db.execute(text("""
                DO $$
                BEGIN
